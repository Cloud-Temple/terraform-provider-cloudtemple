...
BUG FIXES :
  ...
  * Fixed datasource `cloudtemple_iam_features` failing at read time with an `Invalid address to set` error when the feature tree reaches the deepest level the schema declares: a leaf feature at that level still emitted an empty `subfeatures` key, which the deepest schema element does not declare. The `subfeatures` key is now emitted only when sub-features exist.
...
MISCELLANEOUS :
  ...
  * The datasource schema-vs-flatten CI validation now covers every datasource. It is driven by a reflection-based non-zero object filler and a coverage registry asserted complete against the provider's datasource map, so a newly added datasource cannot silently escape the check and the read-breaking class behind #241/#243 is guarded provider-wide.
