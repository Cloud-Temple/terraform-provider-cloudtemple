...
type OpenIaaSHost struct {
	...
	Pool             OpenIaaSPool           `terraform:"pool"`
	Name             string                 `terraform:"name"`
	Uptime           int                    `terraform:"uptime"`
	PowerState       string                 `terraform:"power_state"`
	UpdateData       OpenIaaSHostUpdateData `terraform:"update_data"`
	Memory           OpenIaaSHostMemory     `terraform:"memory"`
	Cpu              OpenIaaSHostCPU        `terraform:"cpu"`
	RebootRequired   bool                   `terraform:"reboot_required"`
	VirtualMachines  []string               `terraform:"virtual_machines"`
}

type OpenIaaSHostCPU struct {
	Cores   int `terraform:"cores"`
	Sockets int `terraform:"sockets"`
}

type OpenIaaSHostMemory struct {
...
type OpenIaaSHostUpdateData struct {
