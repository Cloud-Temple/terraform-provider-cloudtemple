...
// when the adapter is on a VPC network). Mirroring the Phase 1 VPC helpers
// (FlattenFloatingIP), the VPC/static-IP/private-network attributes are
// flattened to FLAT Computed strings, and an absent association collapses to
// "" so every Computed key is always set (never nil, never a panic, #238). The
// NON-deprecated nested privateNetwork{id,name} is used; the deprecated
// top-level privateNetworkId/Name are ignored.
